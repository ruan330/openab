...
| 2026-04-10 | 同步 | — | Public fork sync（squash `27d04c3`，18 檔案 +839/-274）：wiki bootstrap、thread continuity、shutdown broadcast、permission outcome envelope、`cleanup_idle` 修復、移除 dead OAuth refresh |
| 2026-04-10 | 更新 | CLAUDE.md, upstream.md | Public fork sync 正式納入文件同步規則（每次 squash 入 log）；upstream.md 的「同步步驟」補 diverged history 陷阱：`-X theirs` 對非 conflict 的 one-sided additions 無效，改用 `git checkout origin/main -- <paths>` 覆蓋工作樹 |
